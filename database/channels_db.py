...
class ChannelsDatabase:
	...
	def __init__(self, db_connection):
		...
		try:
			...
			self.is_postgres = (info.get('type') == 'postgresql')
		except Exception:
		...
		try:
			with self.db.get_connection() as conn:
				...
				if self.is_postgres:
					...
					cursor.execute('CREATE INDEX IF NOT EXISTS idx_user_channels_channel_id ON user_channels(channel_id)')
				else:
					# SQLite DDL (كما كان سابقاً)
					cursor.execute('''
						CREATE TABLE IF NOT EXISTS user_channels (
	...
	def add_channel(self, user_id: int, chat_id: int, chat_name: str, username: Optional[str] = None, is_admin: bool = False) -> bool:
		...
		try:
			with self.db.get_connection() as conn:
				cursor = conn.cursor()
				if self.is_postgres:
					# Upsert في PostgreSQL
					cursor.execute('''
						INSERT INTO user_channels 
						(user_id, channel_id, channel_name, channel_username, is_admin, updated_at)
					...
					''', (user_id, str(chat_id), chat_name, username, is_admin))
				else:
	...
	def get_user_channels(self, user_id: int) -> List[Dict]:
		...
		try:
			with self.db.get_connection() as conn:
				...
				if self.is_postgres:
					cursor.execute('''
						SELECT channel_id, channel_name, channel_username, is_admin, created_at, updated_at
						FROM user_channels
						WHERE user_id = %s
				...
				if self.is_postgres:
					...
					for key, value in updates.items():
						if key in ['chat_name', 'username', 'is_admin']:
							# خريطة الأعمدة لبوستجرس
							column = 'channel_name' if key == 'chat_name' else ('channel_username' if key == 'username' else 'is_admin')
							set_clauses.append(f"{column} = %s")
							values.append(value)
					if not set_clauses:
						return False
					set_clauses.append("updated_at = CURRENT_TIMESTAMP")
					values.extend([str(chat_id), user_id])
					query = f'''
	...
	def delete_channel(self, chat_id: int, user_id: int) -> bool:
		...
		try:
			with self.db.get_connection() as conn:
				...
				if self.is_postgres:
					cursor.execute('''
						DELETE FROM user_channels WHERE channel_id = %s AND user_id = %s
					''', (str(chat_id), user_id))
				else:
					cursor.execute('''
