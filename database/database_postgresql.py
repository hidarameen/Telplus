...
""")
    ...
    # Header/Footer settings for audio tags

    def get_audio_tag_header_footer_settings(self, task_id: int) -> dict:
        """Fetch header/footer settings for audio tags"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                cursor.execute("""
                    SELECT * FROM task_audio_tag_header_footer_settings WHERE task_id = %s
                """, (task_id,))
                row = cursor.fetchone()
                if row:
                    return {
                        'header_enabled': bool(row.get('header_enabled', False)),
                        'header_text': row.get('header_text') or '',
                        'footer_enabled': bool(row.get('footer_enabled', False)),
                        'footer_text': row.get('footer_text') or '',
                        'apply_to_title': bool(row.get('apply_to_title', True)),
                        'apply_to_artist': bool(row.get('apply_to_artist', True)),
                        'apply_to_album_artist': bool(row.get('apply_to_album_artist', True)),
                        'apply_to_album': bool(row.get('apply_to_album', True)),
                        'apply_to_year': bool(row.get('apply_to_year', False)),
                        'apply_to_genre': bool(row.get('apply_to_genre', True)),
                        'apply_to_composer': bool(row.get('apply_to_composer', True)),
                        'apply_to_comment': bool(row.get('apply_to_comment', True)),
                        'apply_to_track': bool(row.get('apply_to_track', False)),
                        'apply_to_lyrics': bool(row.get('apply_to_lyrics', True))
                    }
        except Exception as e:
            logger.error(f"Error getting audio tag header/footer settings: {e}")
        # Defaults if missing
        return {
    ...
    def update_audio_tag_header_footer_setting(self, task_id: int, setting_name: str, value) -> bool:
        """Update a specific header/footer setting"""
        valid_text = {'header_text', 'footer_text'}
        valid_bool = {
        ...
        if setting_name not in valid_text and setting_name not in valid_bool:
            logger.error(f"Invalid header/footer setting: {setting_name}")
            return False
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_audio_tag_text_cleaning_keywords(self, task_id: int) -> list:
        """Return keywords list for audio tag text cleaning"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
            ...
            logger.error(f"Error getting audio tag text cleaning keywords: {e}")
            return []

    def clear_audio_tag_text_cleaning_keywords(self, task_id: int) -> bool:
        """Clear all cleaning keywords for a task"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    DELETE FROM task_audio_tag_text_cleaning_keywords WHERE task_id = %s
                """, (task_id,))
                conn.commit()
                return True
        except Exception as e:
            logger.error(f"Error clearing audio tag text cleaning keywords: {e}")
            return False

    def get_audio_tag_word_filter_entries(self, task_id: int, filter_type: str) -> list:
        """Get words for word filters (whitelist/blacklist) for a task"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT e.word_or_phrase, e.is_case_sensitive
                    FROM audio_tag_word_filter_entries e
                    WHERE e.filter_id IN (
                        SELECT id FROM task_audio_tag_word_filters
                        WHERE task_id = %s AND filter_type = %s
                    )
                    ORDER BY e.word_or_phrase
                """, (task_id, filter_type))
                return [{
                    'word_or_phrase': row[0],
                    'is_case_sensitive': bool(row[1])
                } for row in cursor.fetchall()]
        except Exception as e:
            logger.error(f"Error getting audio tag word filter entries: {e}")
            return []

    def clear_audio_tag_word_filter_entries(self, task_id: int, filter_type: str) -> bool:
        """Clear all entries for a specific word filter type"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
            ...
            logger.error(f"Error clearing audio tag word filter entries: {e}")
            return False
    ...
    def add_audio_tag_word_filter_entry(self, task_id: int, filter_type: str, word_or_phrase: str, is_case_sensitive: bool = False) -> bool:
        """Add a word/phrase to a filter list"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                # Ensure parent filter row exists
                cursor.execute("""
                ...
                # Fetch id
                cursor.execute("""
                    SELECT id FROM task_audio_tag_word_filters WHERE task_id = %s AND filter_type = %s
                """, (task_id, filter_type))
            ...
            logger.error(f"Error adding audio tag word filter entry: {e}")
            return False

    def get_audio_tag_text_replacement_entries(self, task_id: int) -> list:
        """Return text replacement entries for a task"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT e.find_text, e.replace_text, e.is_case_sensitive, e.is_whole_word
                    FROM audio_tag_text_replacement_entries e
                    WHERE e.replacement_id IN (
                        SELECT id FROM task_audio_tag_text_replacements WHERE task_id = %s
                    )
                    ORDER BY e.find_text
                """, (task_id,))
                return [{
                    'find_text': row[0],
                    'replace_text': row[1],
                    'is_case_sensitive': bool(row[2]),
                    'is_whole_word': bool(row[3])
                } for row in cursor.fetchall()]
        except Exception as e:
            logger.error(f"Error getting audio tag text replacement entries: {e}")
            return []

    def clear_audio_tag_text_replacements(self, task_id: int) -> bool:
        """Clear all text replacement entries for a task"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
            ...
            logger.error(f"Error clearing audio tag text replacement entries: {e}")
            return False
    ...
    def add_audio_tag_text_replacement_entry(self, task_id: int, find_text: str, replace_text: str, is_case_sensitive: bool = False, is_whole_word: bool = False) -> bool:
        """Add a text replacement entry for a task"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
            ...
            logger.error(f"Error adding audio tag text replacement entry: {e}")
            return False
